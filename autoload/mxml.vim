...
function! mxml#complete(findstart, base)
  if a:findstart
    let line = getline('.')
    let start = col('.') - 1
...
function! s:attr_completion(tag, base, res)
  ...
  for member in item.members
    if member.name =~ '^' . a:base && !flexapi#isMethod(member)
...
function! s:bind_attr_completion(tag, base, res)
  if !flexapi#isBindingExist(a:tag)
    return
  endif

  let item = flexapi#getBinding(a:tag)
  for member in item.members
  ...
  for member in s:names()
    call insert(a:res, flexapi#member_to_compitem('', member), 0)
  endfor
endfunction
...
function! s:value_completion(tag, prop, base, res)
  ...
  if flexapi#isEnumExist(mtype.class)
    let enum = flexapi#getEnum(mtype.class)
    for member in enum.members
      if member.name =~ '^' . a:base
        call add(a:res, flexapi#member_to_compitem(member.name, member))
      endif
    endfor
  endif
...
function! s:binding_completion(base, res)
  for key in keys(s:binding)
    if key =~ a:base
      let item = flexapi#getBinding(key)
      call add(a:res, flexapi#member_to_compitem(key, item))
    endif
  endfor
  return flexapi#prop('', '')
endfunction
...
function! s:names()
  ...
  for line in lines
    ...
    call add(names, flexapi#prop(line[ start+1 : end-1 ], class))
  endfor
  return names
endfun

let [ s:class, s:enum, s:binding ] = flexapi#classes()
